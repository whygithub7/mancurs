...
let isTouch = false; // Variabile per tracciare i dispositivi touch
...
window.addEventListener('DOMContentLoaded', function() {
    initializeQuiz();
    initializeForm();
    initializeProductDetails(); // Aggiungi questa riga
    startCountdown();
    renderProductsGrid(); // Chiamiamo la funzione di rendering dopo il caricamento del DOM
    ...
    // Новый обработчик для всех ссылок в футере, кроме select-product-btn и cta-button
    document.querySelectorAll('.footer a:not(.select-product-btn):not(.cta-button)').forEach(link => {
...
// Funzioni popup rimosse, viene utilizzato un banner
...
function startCountdown() {
    ...
    countdownInterval = setInterval(() => {
        ...
        // Verificare la presenza di elementi prima dell'aggiornamento
...
function initializeQuiz() {
    ...
    newQuizOptions.forEach(option => {
        option.addEventListener('click', function() {
            ...
            // Переходим к следующему вопросу мгновенно
            nextQuestion();
        });
    });
    
    // Сбрасываем отображение вопросов
...
function showQuizResult() {
    ...
    setTimeout(() => {
        ...
        // Скрываем все вопросы, но НЕ quiz-container (чтобы результат остался видимым)
        ...
        // Скрываем весь quiz блок, теперь нужен только order
        document.getElementById('quiz').style.display = 'none';
        
        // Убираем класс quiz-only с body, чтобы CSS не мешал
        document.body.classList.remove('quiz-only');
...
function updateSelectedProduct(index) {
    ...
    if (subtotalEl) subtotalEl.textContent = product.price;
    if (totalEl) totalEl.textContent = product.price;
        ...
        card.innerHTML = `
            ...
            <div class="product-info-new">
                ...
                <div class="price-container-new">
                    <span class="original-price-new">${product.originalPrice}</span>
                    <span class="current-price-new">${product.price}</span>
                </div>
                <p class="product-description-new">${product.description}</p>
                <button class="cta-button details-btn-new" data-product="${idx}">Dettagli</button>
            </div>
        `;
        card.querySelector('.details-btn-new').onclick = () => openProductPage(idx);
        card.onclick = (e) => {
            if (!e.target.classList.contains('details-btn-new')) openProductPage(idx);
        };
        grid.appendChild(card);
    });
}

// Новый модал для деталей набора
function openProductModal(idx) {
    const product = products[idx];
    if (!product) return;
    // Собираем все фото для набора
    let images = getProductImages(product);
    let modal = document.createElement('div');
    modal.className = 'modal-overlay-new';
    modal.innerHTML = `
        <div class="modal-content-new">
            <button class="close-btn-new">&times;</button>
            <div class="modal-slider-new">
                ${images.map((img, i) => `<img src="${img}" class="modal-img-new${i===0?' active':''}" data-idx="${i}">`).join('')}
                <div class="modal-slider-nav">
                    <button class="modal-nav-btn" id="modalPrev">&#8592;</button>
                    <button class="modal-nav-btn" id="modalNext">&#8594;</button>
                </div>
            </div>
            <div class="modal-product-details-new">
                <h3>${product.name}</h3>
                <div class="modal-price-container-new">
                    <span class="original-price-new">${product.originalPrice}</span>
                    <span class="current-price-new">${product.price}</span>
                </div>
                <p class="modal-description-new">${product.description}</p>
                <button class="cta-button select-product-btn" data-product="${idx}">Scegli Questo Set</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
    // Закрытие
    modal.querySelector('.close-btn-new').onclick = () => modal.remove();
    modal.onclick = e => { if (e.target === modal) modal.remove(); };
    // Слайдер
    let current = 0;
    const imgs = modal.querySelectorAll('.modal-img-new');
    function showImg(i) {
        imgs.forEach((img, idx) => img.classList.toggle('active', idx === i));
        current = i;
    }
    modal.querySelector('#modalPrev').onclick = e => { e.stopPropagation(); showImg((current-1+imgs.length)%imgs.length); };
    modal.querySelector('#modalNext').onclick = e => { e.stopPropagation(); showImg((current+1)%imgs.length); };
}

// Получить все фото для набора (по папке)
function getProductImages(product) {
    // Маппинг id -> массив фото (добавьте все нужные фото)
    const imagesMap = {
        1: [
            'assets/images/viagra/viagra_connect_bundle.webp',
            'assets/images/viagra/viagra_connect_bundle_trolley.jpg',
            'assets/images/viagra/via2.avif',
            'assets/images/viagra/via3.png',
            'assets/images/viagra/via4.webp',
            'assets/images/viagra/via5.webp',
        ...
        5: [
            ...
            'assets/images/intim/gum2.webp',
            'assets/images/intim/gum3.webp',
            'assets/images/intim/gum4.webp',
            'assets/images/intim/lubr.avif',
            'assets/images/intim/couples_libido_gummies_main.webp',
            'assets/images/intim/in0.webp',
            'assets/images/intim/in1.jpg',
            'assets/images/intim/in2.webp',
            'assets/images/intim/dado_kamasutra_sex_toys.webp',
        ],
    };
    return imagesMap[product.id] || [product.image];
}

// SPA: показать страницу набора
function openProductPage(idx) {
    // Исправляем индекс: products.id - 1
    const product = products[idx];
    const dIdx = product.id - 1;
    const details = detailedProducts[dIdx];
    const images = productImages[dIdx] || [product.image];
    const main = document.body;
    if (!window._mainContent) {
        window._mainContent = main.innerHTML;
    }
    // Оставляем только 3-4 ключевых features
    const features = details.features.slice(0, 4);
    // Компоненты — горизонтальный скролл
    const components = (details.components||[]);
    
    // Скролл в начало страницы при открытии
    window.scrollTo({ top: 0, behavior: 'auto' });
    
    main.innerHTML = `
    <div class="product-page-intima">
        <header class="header mini">
            <div class="container">
                <button class="back-btn-new">&#8592;</button>
                <div class="logo mini"><h1>Collezione Intima</h1></div>
            </div>
        </header>
        <section class="product-section-intima">
            <div class="slider-block-intima">
                <div class="product-slider-intima" id="productSlider">
                    ${images.map((img, i) => `<img src="${img}" class="slider-img-intima${i===0?' active':''}" data-idx="${i}">`).join('')}
                </div>
                <div class="slider-dots-intima">
                    ${images.map((_, i) => `<span class="slider-dot-intima${i===0?' active':''}" data-idx="${i}"></span>`).join('')}
                </div>
                <button class="slider-nav-btn-intima left" id="sliderPrev">&#8592;</button>
                <button class="slider-nav-btn-intima right" id="sliderNext">&#8594;</button>
            </div>
            <div class="product-details-intima">
                <div class="title-price-row">
                  <h2 class="product-title-intima">${details.name}</h2>
                  <div class="price-block-intima">
                      <span class="original-price-intima">${details.originalPrice}</span>
                      <span class="current-price-intima">${details.currentPrice}</span>
                  </div>
                </div>
                <div class="desc-block-intima">
                    <p class="product-short-desc-intima">${details.description}</p>
                </div>
                <div class="features-block-intima">
                    <h4><span class="icon-feat">&#10024;</span> Vantaggi principali</h4>
                    <ul class="features-list-intima">
                        ${features.map(f => `<li><span class="icon-feat">✔️</span> ${f}</li>`).join('')}
                    </ul>
                </div>
                <div class="components-block-intima">
                    <h4><span class="icon-comp">🎁</span> Cosa c'è nel set</h4>
                    <div class="components-list-intima horizontal-scroll">
                        ${components.map(c => `
                            <div class="component-card-intima">
                                <div class="component-title-intima">${c.name}</div>
                                <div class="component-desc-intima">${c.description}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
                <div class="product-navigation-intima">
                    <button class="nav-btn-intima prev-product" data-direction="prev">
                        <span class="nav-icon">←</span>
                        <span class="nav-text">Set Precedente</span>
                    </button>
                    <button class="nav-btn-intima next-product" data-direction="next">
                        <span class="nav-text">Set Successivo</span>
                        <span class="nav-icon">→</span>
                    </button>
                </div>
            </div>
        </section>
        <button class="cta-button-intima sticky-btn-intima" id="quizBtn">SCEGLI QUESTO SET</button>
    </div>
    `;
    // Назад
    document.querySelector('.back-btn-new').onclick = () => {
        main.innerHTML = window._mainContent;
        // Не рендерим главную, если активен квиз
    ...
    // Слайдер
    let current = 0;
    const imgs = document.querySelectorAll('.slider-img-intima');
    const dots = document.querySelectorAll('.slider-dot-intima');
    function showImg(i) {
        imgs.forEach((img, idx) => img.classList.toggle('active', idx === i));
        dots.forEach((dot, idx) => dot.classList.toggle('active', idx === i));
        current = i;
    }
    document.getElementById('sliderPrev').onclick = e => { e.stopPropagation(); showImg((current-1+imgs.length)%imgs.length); };
    document.getElementById('sliderNext').onclick = e => { e.stopPropagation(); showImg((current+1)%imgs.length); };
    dots.forEach(dot => dot.onclick = () => showImg(Number(dot.dataset.idx)));
    // Свайп для мобилок
    let startX = null;
    ...
    slider.addEventListener('touchend', e => {
        ...
        if (Math.abs(dx) > 40) {
            if (dx < 0) showImg((current+1)%imgs.length);
            else showImg((current-1+imgs.length)%imgs.length);
        }
        startX = null;
    });
    // Кнопка запускает квиз сразу
    ...
    // Обработчики навигации между продуктами
    const prevBtn = document.querySelector('.nav-btn-intima.prev-product');
    const nextBtn = document.querySelector('.nav-btn-intima.next-product');
    
    if (prevBtn) {
        prevBtn.onclick = function() {
            const prevIdx = (idx - 1 + products.length) % products.length;
            openProductPage(prevIdx);
        };
    }
    ...
    if (nextBtn) {
        nextBtn.onclick = function() {
            ...
            openProductPage(nextIdx);
        };
    }
}

// --- Квиз: скрытие всего контента кроме квиза (JS-метод) ---
function showQuizOnly() {
    document.body.classList.add('quiz-only');
    document.getElementById('quiz').style.display = 'block';
    // Навсегда скрываем hero и products при запуске квиза
    document.querySelector('.hero').style.display = 'none';
    document.getElementById('products').style.display = 'none';
}
function showAllExceptQuiz() {
    document.body.classList.remove('quiz-only');
    document.getElementById('quiz').style.display = 'none';
    // НЕ показываем hero и products после квиза - они остаются скрытыми
}
// --- Добавляем кнопку назад в квиз ---
function addQuizBackButton() {
    const quizContainer = document.querySelector('.quiz-container');
    if (quizContainer && !document.getElementById('quizBackBtn')) {
        const backBtn = document.createElement('button');
        backBtn.id = 'quizBackBtn';
        backBtn.className = 'quiz-back-btn';
        backBtn.innerHTML = '← Torna alla selezione';
        backBtn.onclick = () => {
            showAllExceptQuiz();
...
// --- Глобальный индекс выбранного товара ---
window.selectedProductIndex = 0;
// --- Кнопка запуска квиза и выбор товара ---
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.cta-button-intima, .select-product-btn').forEach(btn => {
        btn.onclick = function(e) {
            // Определяем индекс выбранного товара
...
// --- Обновление order-summary и скрытых полей формы ---
function updateOrderSummary(idx) {
    const product = products[idx];
    if (!product) return;
    
    // Используем унифицированную функцию updateSelectedProduct
    updateSelectedProduct(idx);
    
    // Дополнительно обновляем selectedProductIndex глобально
    selectedProductIndex = idx;
    window.selectedProductIndex = idx;
}
// --- При возврате с квиза обновляем order-summary ---
function onQuizComplete() {
    showAllExceptQuiz();
    renderProductsGrid();
    updateOrderSummary(window.selectedProductIndex);
}
